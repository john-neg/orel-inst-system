...
@bp.route("/library_choose_plan", methods=["GET", "POST"])
@login_required
def library_choose_plan():
    ...
    return render_template(
        "library/library_choose_plan.html", active="library", title="Загрузка списка литературы", form=form
    )
...
def library_upload(plan_id):
    ...
    return render_template(
        "library/library_upload.html", active="library", title="Загрузка списка литературы", form=form, plan_name=plan_name
    )
...
@bp.route("/library_np_choose_plan", methods=["GET", "POST"])
@login_required
def library_np_choose_plan():
    form = ChoosePlan()
    form.edu_spec.choices = list(education_specialty().items())
    if request.method == "POST":
        edu_spec = request.form.get("edu_spec")
        ...
        if request.form.get("edu_plan") and form.validate_on_submit():
            edu_plan = request.form.get("edu_plan")
            return redirect(url_for("library.library_np_upload", plan_id=edu_plan))
        return render_template(
            "library/library_choose_plan.html",
            active="library",
    ...
    return render_template(
        "library/library_choose_plan.html", active="library", title="Загрузка списка научной продукции", form=form
    )
...
def library_np_upload(plan_id):
    ...
    if request.method == "POST":
        if request.form.get("library_load_temp"):
            return redirect(
                url_for("main.get_temp_file", filename="library_np_load_temp.xlsx")
            )
        if request.form.get("library_plan_content"):
            return redirect(url_for("library.library_np_export", plan_id=plan_id))
        if request.files["file"]:
            file = request.files["file"]
            if file and allowed_file(file.filename):
                filename = file.filename
                ...
                if request.form.get("library_check"):
                    ...
                    return redirect(
                        url_for(
                            "library.library_np_check", plan_id=plan_id, filename=filename
                        )
                    )
                if request.form.get("library_update"):
                    # Загрузка списка литературы
                    return redirect(
                        url_for(
                            "library.library_np_update", plan_id=plan_id, filename=filename
                        )
                    )
    return render_template(
        "library/library_np_upload.html", active="library", title="Загрузка списка научной продукции", form=form, plan_name=plan_name
    )


@bp.route("/library_np_check/<int:plan_id>/<string:filename>", methods=["GET", "POST"])
@login_required
def library_np_check(plan_id, filename):
    ...
    if request.method == "POST":
        ...
        if request.form.get("library_update"):
            return redirect(
                url_for("library.library_np_update", plan_id=plan_id, filename=filename)
            )
    # Check if program in uploaded file
    work_programs, no_data = [], []
    for wp_id in plan.work_programs:
...
@bp.route("/library_np_update/<int:plan_id>/<string:filename>", methods=["GET", "POST"])
@login_required
