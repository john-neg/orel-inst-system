...
for data in documents_data:
    if not doc_service.get(name=documents_data[data].get("name")):
...
for rate in rate_data:
    if not rate_service.get(slug=make_slug(rate_data[rate].get("name"), prefix="rate_")):
...
for data in increase_data:
    if not increase_service.get(name=increase_data[data].get("name")):
...
for data in pension_duty_data:
    if not pension_duty_service.get(name=pension_duty_data[data].get("name")):
...
for data in global_coefficient_data:
    if not global_service.get(slug=make_slug(global_coefficient_data[data].get("name"), prefix="coeff_")):
